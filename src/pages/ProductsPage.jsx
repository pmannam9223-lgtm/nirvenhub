...
//       const prods = await fetchProducts({ ordering: 'order' });
//       setProducts(prods);
//     };
//     loadData();
//   }, []);
...
//             >
//               <span className={`text-lg font-black text-primary transition-all text-primary`}>
//                 All Products
//               </span>
...
const ProductsPage = () => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    const getMinPrice = (p) => p.variants && p.variants.length > 0 ? Math.min(...p.variants.map(v => parseFloat(v.selling_price))) : 0;
    result = result.filter(p => getMinPrice(p) <= priceRange);
    if (sortBy === 'price-low') result.sort((a, b) => getMinPrice(a) - getMinPrice(b));
    if (sortBy === 'price-high') result.sort((a, b) => getMinPrice(b) - getMinPrice(a));
    if (sortBy === 'rating') result.sort((a, b) => b.rating - a.rating);
    return result;
  }, [products, query, sortBy, priceRange, offersOnly]);
  const FilterContent = ({ isMobile = false }) => (
