...
export const ProductCard = ({ product }) => {
  ...
  // Safe fallback values if a product has no variants created yet
  const displayPrice = selectedVariant ? selectedVariant.selling_price : 0;
  const displayOriginalPrice = selectedVariant ? selectedVariant.original_price : null;
  const displayDiscount = selectedVariant ? selectedVariant.discount_percentage : 0;
  return (
    <Link 
      to={`/product/${product.id}`} 
      className="group relative flex flex-col gap-6 animate-fade-up"
    >
      <div className="relative aspect-[4/5] rounded-[32px] overflow-hidden bg-[#F2F4F2] border border-white/40 shadow-[0_20px_40px_rgba(27,60,26,0.05)] transition-all duration-700 group-hover:shadow-[0_40px_80px_rgba(27,60,26,0.12)] group-hover:translate-y-[-8px]">
        ...
        <div className="absolute top-5 left-5 z-20 flex flex-col gap-2">
          ...
          {selectedVariant && !isOutOfStock && product.in_offer && (
            <div className="bg-white/90 backdrop-blur-md text-primary text-[10px] font-black px-3 py-1.5 rounded-full shadow-sm">
              SPECIAL SALE
            </div>
          )}
          {selectedVariant && !isOutOfStock && displayDiscount > 0 && (
            <div className="bg-red-500/90 backdrop-blur-md text-white text-[10px] font-black px-3 py-1.5 rounded-full shadow-sm">
              {displayDiscount}% OFF
            </div>
          )}
      ...
      <div className="px-2 space-y-3">
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="flex flex-col flex-1">
             ...
             {displayOriginalPrice && displayOriginalPrice > displayPrice && <p className="text-xs text-gray-300 line-through font-bold">£{displayOriginalPrice}</p>}
             ...
             <div className="mt-2" onClick={handleSelectClick}>
                {variants.length > 1 ? (
                  ...
                  <span className="text-[10px] font-black text-red-400 uppercase tracking-widest">Missing Variants</span>
                )}
             </div>
          </div>
